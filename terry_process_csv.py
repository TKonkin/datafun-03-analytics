"""
Process a CSV file on business innovation to analyze the `value` column and save statistics.
"""
...
def analyze_value_score(file_path: pathlib.Path) -> dict:
    """Analyze the value column to calculate min and max"""
    try:
...
def process_csv_file():
    ...
    input_file = pathlib.Path(fetched_folder_name, "bus_innovation.csv")
    output_file = pathlib.Path(processed_folder_name, "value_score.txt")
    
    stats = analyze_value_score(input_file)
    output_file.parent.mkdir(parents=True, exist_ok=True)
    
    with output_file.open('w') as file:
        file.write("Value Score Statistics:\n")
